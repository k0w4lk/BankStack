...
local ignore_options = {
	...
	args = {
		...
		add = {
			...
			get = false,
			set = function(info, v)
				local bag, slot = string.match(v, "^([-%d]+)%s?(%d*)$")
				if bag and tonumber(slot) then
		...
		remove = {
			...
			set = function(info, v)
				...
				if bag and (not tonumber(slot)) then
					...
					core.announce(0, bag.." no longer ignored.", 1, 1, 1)
				end
			end,
			usage = "[bag] [slot] (see http://wowwiki.com/BagID)",
		},
